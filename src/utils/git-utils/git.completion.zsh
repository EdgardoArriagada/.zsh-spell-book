...
ZSB_GIT_NEW_FILES="${zsb}_isGitRepo && git status --short | grep -E '(^ \?)|(^\?{2})' | sed s/^...//"
ZSB_GIT_UNSTAGED_AND_NEW_FILES="${zsb}_isGitRepo && git status --short | grep -E '(^ [MARCD\?])|(^[MARCD\?]{2})' | sed s/^...//"
