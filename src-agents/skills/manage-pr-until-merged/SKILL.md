...
2. On a new comment or substantive review by an actor other than your login, spawn one comment worker with model `gpt-6-sol`. Tell it to invoke `$solve-pr-comments <PR_URL>`, evaluate and address the outstanding feedback, stage only its changes, commit, and push the PR head branch. The user explicitly authorized that push, so it must perform it before replying to threads that needed code changes; this overrides the referenced skill's wait-for-user step. Do not start a second mutating worker while one runs. Queue later comments and run another worker after the current one finishes if feedback remains.
3. If `CHAT_URL` exists, post exactly `comentarios resueltos` once after each comment worker finishes successfully with its required fixes pushed and replies completed. Send it as webhook JSON `{"text":"comentarios resueltos"}` over HTTPS. Do not post it for failed workers or pipeline work.
4. Watch current-head CI with `poll-pr-pipeline <PR_URL>` (also without `--tmux`), restarting it after each push. If checks fail, spawn a worker with the goal **"solve pr pipeline"** for this PR. Tell it that it can use `poll-pr-pipeline <PR_URL>`, must inspect the failed checks, fix the root cause, verify the fix, commit, and push its own changes. Serialize it with the comment worker. Retry temporary command failures; do not repeatedly dispatch workers for the same failed head without a new diagnosis or change.
5. On each `PR ready to merge` signal, successful CI result, and worker completion, recheck the PR's current head, open state, review status, and current-head CI. Require GitHub merge readiness and passing applicable checks; zero reported checks do not prove CI passed. Require no pending worker or unaddressed feedback. Then run `gh pr merge <PR_URL> --squash --match-head-commit <HEAD_SHA>`. If readiness changes, keep watching. Verify the PR state is `MERGED` before stopping. If GitHub blocks merging or a worker cannot resolve a failure, report the specific blocker and seek direction; otherwise continue until merged, with no time limit.
